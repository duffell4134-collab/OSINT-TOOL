...
// 1. Username Single / Batch Check API
app.post('/api/osint/username-check', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      // Detection logic based on status code and string matching
...
// 2. DNS-over-HTTPS Lookup (Cloudflare & Google DoH)
app.post('/api/osint/dns-lookup', async (req: Request, res: Response) => {
  try {
    ...
    // Analyze Security Posture (SPF, DMARC, DNSSEC)
    const txtRecords = results['TXT'] || [];
    const hasSPF = txtRecords.some((r: any) => r.data && r.data.includes('v=spf1'));
...
app.post('/api/osint/subdomains', async (req: Request, res: Response) => {
  try {
    const { domain } = req.body;
    if (!domain) {
      return res.status(400).json({ error: 'Domain is required' });
...
// 4. IP Geolocation, ASN & Network Intelligence
app.post('/api/osint/ip-lookup', async (req: Request, res: Response) => {
  try {
    const { ip } = req.body;
    if (!ip) {
    ...
    // Check if Tor exit node (public list or pattern)
    return res.json({
      ip: cleanIp,
      success: true,
      data: {
...
// 5. RDAP / WHOIS Lookup
app.post('/api/osint/rdap-lookup', async (req: Request, res: Response) => {
  try {
    const { domain } = req.body;
    if (!domain) {
      return res.status(400).json({ error: 'Domain is required' });
    ...
    try {
      ...
      if (Array.isArray(rdapData.events)) {
        rdapData.events.forEach((ev: any) => {
      ...
      if (Array.isArray(rdapData.nameservers)) {
        rdapData.nameservers.forEach((ns: any) => {
...
// 6. Email OSINT & Identity Verification
app.post('/api/osint/email-recon', async (req: Request, res: Response) => {
  try {
...
// 7. Wayback Machine CDX API Explorer
app.post('/api/osint/wayback-snapshots', async (req: Request, res: Response) => {
  try {
    const { url } = req.body;
    if (!url) {
...
// 8. GitHub Profile & Repo OSINT Recon
app.post('/api/osint/github-recon', async (req: Request, res: Response) => {
  try {
...
// 9. Fetch and Parse External JSON (WhatsMyName, Sherlock, ARF)
app.post('/api/osint/fetch-json', async (req: Request, res: Response) => {
  try {
    const { url } = req.body;
    if (!url) {
...
// 10. AI OSINT Intelligence Analyst (Gemini 3.7 Flash)
app.post('/api/osint/ai-analyze', async (req: Request, res: Response) => {
  try {
